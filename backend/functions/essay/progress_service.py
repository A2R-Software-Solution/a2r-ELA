...
# ─── Streak Bonus XP Thresholds ───────────────────────────────────────────────
# Awarded once per submission when the streak hits the threshold.
...
class ProgressService:
    ...
    def update_progress_after_submission(
        ...
        Also calculates streak_bonus_xp if the new streak hits a
        milestone (7-day or 30-day). The bonus is returned in the
        result dict so reward_engine can fold it into the XP total —
        progress_service never writes XP itself.
        ...
        Returns:
            Updated progress data including streak_bonus_xp
        """
        progress_ref = self.db.collection(
            settings.COLLECTION_USER_PROGRESS
        ).document(user_id)
