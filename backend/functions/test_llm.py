...
def test_llm_connection(req: https_fn.Request) -> https_fn.Response:
    ...
    results["api_key_present"] = bool(settings.GROQ_API_KEY)
    results["api_key_length"] = len(settings.GROQ_API_KEY) if settings.GROQ_API_KEY else 0
    results["base_url"] = settings.GROQ_BASE_URL
