...
export const useHome = () => {
  ...
  // ✅ Recent Essays
  const loadRecentEssays = useCallback(async () => {
    try {
      const response = await apiService.getUserSubmissions(5);
      const submissions = (response.data?.data as any)?.submissions ?? [];
      ...
      setUiState(prev => ({ ...prev, recentEssays }));
    } catch (error) {
      console.error('useHome: failed to load recent essays:', error);
    }
  }, []);
  ...
  const loadHomeData = useCallback(() => {
    const categories: CategoryUiModel[] = [
      ...
      { id: '3', title: 'Exam Prep', isSelected: false },
    ...
    const features: FeatureUiModel[] = [
      ...
      { id: 'ExPrep',  title: 'Exam Prep',     subtitle: 'Practice Questions', iconRes: '📚', colorRes: null },
    ...
    setUiState(prev => ({ ...prev, categories, features, recentCourses, streak }));
  }, []);
